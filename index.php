...
if (isset($_GET['new'])) {
  ...
  if (!isset($shipList[$lore['ship']])) {
    usError("Invalid ship type! Please try again.");
    $validationErrors += 1;
  }
  if (strlen($lore['new_ship']) > 40) {
    usError("Name too long! Please try again.");
    $validationErrors += 1;
  }
  if ($validationErrors == 0) {
...
if ($no_rows == 1) {
  ...
  <h3>Returning all Registered Ships for: "<?= echousername($user->data()->id); ?>
    <?= nl2br("</h3>"); ?>
    <table class="table table-dark table-striped table-bordered table-hover table-responsive-md">
      ...
      <?php while ($row = $result->fetch_assoc()) {
        ...
        <?php $stmtship = $mysqli->prepare("SELECT * FROM ships WHERE seal_ID = ? AND ID = ? AND del_flag <> 1");
        ...
        $stmtship->close(); ?>
        <div class="modal fade" id="me<?= $field1name ?>" tabindex="-1">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <form action="?edit" method="post">
                  ...
                  <div class="input-group mb-3">
                    ...
                    <select name="ship" class="custom-select" id="inputGroupSelect01" placeholder="Ship Type" required>
                      <?php foreach ($shipList as $shipId => $shipName) {
                  ...
                  <div class="input-group mb-3">
                    <input type="url" name="link" id="link" class="form-control" value="<?= $shipLink ?>" placeholder="Coriolis Shortlink (Optional) https://s.orbis.zone/" pattern="(https?:\/\/(.+?\.)?orbis\.zone(\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&\'\(\)\*\+,;\=]*)?)" size="30">
                  </div>
